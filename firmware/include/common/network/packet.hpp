...
public:
    ...
    GS_NODISCARD const PacketHeader& header() const noexcept
    {
        return header_;
    }
    GS_NODISCARD const uint8_t* payload() const noexcept
    {
        return payload_.data();
    }
    GS_NODISCARD uint16_t payload_length() const noexcept
    {
        return header_.payload_length;
    }

    GS_NODISCARD bool is_valid() const noexcept
    {
        return is_valid_;
    }
