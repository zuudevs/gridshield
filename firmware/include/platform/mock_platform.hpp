...
public:
  ...
  // Test helper
  void simulate_trigger(uint8_t pin, bool state) { pin_states_[pin] = state; }
...
public:
  ...
  core::Result<size_t> send(const uint8_t *data,
                            size_t length) noexcept override {
    if (GS_UNLIKELY(!initialized_ || !connected_)) {
      return core::Result<size_t>(
          GS_MAKE_ERROR(core::ErrorCode::NetworkDisconnected));
    }
  ...
  core::Result<size_t> receive(uint8_t *buffer, size_t max_length,
    ...
    if (rx_buffer_.empty()) {
      return core::Result<size_t>(
          GS_MAKE_ERROR(core::ErrorCode::NetworkTimeout));
    }
