...
void app_main(void)
{
    ...
    if (wdt_result.is_error()) {
        ESP_LOGW(TAG,
                 "Watchdog init failed (code=%d) — continuing",
                 static_cast<int>(wdt_result.error().code));
    } else {
    ...
    ESP_LOGI(
        TAG, "Config loaded (meter_id=0x%llx)", static_cast<unsigned long long>(config.meter_id));
    ...
    if (save_res.is_error()) {
        ESP_LOGW(
            TAG, "Failed to save config to NVS (code=%d)", static_cast<int>(save_res.error().code));
    }
